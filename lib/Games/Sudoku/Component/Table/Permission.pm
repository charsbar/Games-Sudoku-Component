package Games::Sudoku::Component::Table::Permission;
{
  ...
  sub allowed {
    ...
    foreach my $ct (1..$this->{size}) {
      push @allowed, $ct if $this->_is_allowed($row, $col, $ct);
    }
    # Don't be too picky if we have enough candidates.
    return @allowed if @allowed > 3;

    # XXX: Sometimes there's only one cell left that can hold
    # a certain value, even if the cell has other candidates.

    foreach my $ct (@allowed) {
      ROW_CHECK:
      {
        foreach my $c (1..$this->{size}) {
          next if $c == $col;
          last ROW_CHECK if $this->_is_allowed($row, $c, $ct);
        }
        return ($ct);
      }
      COL_CHECK:
      {
        foreach my $r (1..$this->{size}) {
          next if $r == $row;
          last COL_CHECK if $this->_is_allowed($r, $col, $ct);
        }
        return ($ct);
      }
      BLOCK_CHECK:
      {
        my $h = $this->{block_height};
        my $w = $this->{block_width};
        my $top  = int(($row - 1) / $h) * $h;
        my $left = int(($col - 1) / $w) * $w;
        foreach my $r (($top + 1) .. ($top + $h)) {
          foreach my $c (($left + 1) .. ($left + $w)) {
            next if $r == $row && $c == $col;
            last BLOCK_CHECK if $this->_is_allowed($r, $c, $ct);
          }
        }
        return ($ct);
      }
  ...
  sub is_allowed {
    ...
    my @allowed = $this->allowed($row, $col);

    return scalar grep { $_ == $value } @allowed;
  }
